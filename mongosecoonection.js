...
app.get("/signup", (req, res) => {
    ...
    // logindata.find({}, 'name', function (err, someValue) {
    //     if (err) console.log(err);
    //     const params = { 'allnames': someValue };
    //     console.log(params);
    //     res.status(200).render('index', params);
...
app.get("/confmail", (req, res) => {
    ...
    var fsjson = fs.readFile(__dirname + "/" + "users.json", 'utf8', function (err, data) {
        ...
        for(x in data){
            if(data[x]["id"] == idconf){
                ...
                res.cookie("userData", cookiedata, { httpOnly: true });
    ...
    // console.log(req.query);
    // const getidconf = req.query.idconf;
    // logindata.find({ idconf: getidconf }, function (err, someValue) {
    //     if (err) console.log(err);
    //     var cookiedata = someValue[0];
    //     const params = { "logindata": cookiedata, "display": "none" };
    //     console.log("my cookie data", cookiedata);
    //     res.cookie("userData", cookiedata, { httpOnly: true });
    //     res.status(200).render('confmail', params);
...
//     return new Promise(resolve => {
//         setTimeout(function () {
//             resolve(logindata.find({ name: myname }, function (err, someValue) {
//                 if (err) console.log(err);
//                 const namedata = someValue;
//                 console.log("name searched in db for confmail", namedata);
//                 let outtowrite = `You are ${name}, and your pass is ${psd}, 
//                     and you have just submitted a form created by burn_chat using nodejs click this link to conf your mail
//                     http://127.0.0.1:8000/confmail?idconf=` + namedata[0].idconf;
